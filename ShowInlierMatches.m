...
% following pattern, where the x indicates that the number is not contained in
...
for i=1:Num_Images
    for j= i+1:Num_Images
        id = id+1;
        Matches(i,j) = 2 * inlier(id)/(keypoints(i) + keypoints(j));
        Matches(j,i) = 2 * inlier(id)/(keypoints(i) + keypoints(j));
